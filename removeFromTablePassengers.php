<html><head><title>Add To Table Screen</title></head><body>
  <h1 style="text-align:center">Remove Passenger</h1>
  <form action="" method="post">
    <table align="center">
      <tr>
        <td align="right">Enter their Passport Number: </td>
...
<?php
  ...
  if (isset($_POST['remove'])) {
    $sql = "DELETE FROM $table WHERE passportNumber = \"$passportNumber\"";
    if (mysqli_query($con, $sql)) {
      if (mysqli_affected_rows($con) > 0) {
        echo "<p align=\"center\">Successfully removed Passenger with Passport Number: $passportNumber.</p>";
      } else {
        echo "<p align=\"center\">No Passenger to remove with that Passport Number.</p>";
      }
    } else {
